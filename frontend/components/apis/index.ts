...
// function wrappedProvider(provider: Web3Provider) {
...
function contractInstances(props: InstanceProps) {
  ...
  if(!provider) alert('Provider not ready');
  const vault_ins = new Contract(vaultAddr, vaultAbi, provider?.getSigner());
  const vault_ins_noSigner = new Contract(vaultAddr, vaultAbi, provider);
  const token_ins = new Contract(tokenAddr, tokenAbi, provider?.getSigner());
  const token_ins_noSigner = new Contract(tokenAddr, tokenAbi, provider);
